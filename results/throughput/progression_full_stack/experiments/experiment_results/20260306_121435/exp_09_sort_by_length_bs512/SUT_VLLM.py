# EXP 09: +sort queries by input token length in issue_queries()
# CHANGE: query_samples sorted by len(input_ids) before batching
# Dataset token lengths: min=79, max=2540, mean=870 — high variance
# Without sorting: long sequences force shorter ones to wait in same batch
# With sorting: similar-length sequences batched together, less padding waste
# Observed effect: mean latency -24% (320s → 244s), throughput +0.7%
...
class SUT:
    ...
    def issue_queries(self, query_samples):
        ...
        # CHANGE: sort by input token length to reduce padding waste per batch
