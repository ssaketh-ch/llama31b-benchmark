# STK_09_BITSANDBYTES: quantization='bitsandbytes' (int8 weight emulation).
# Added on top of stk_08: replaces quantization=fp8 with bitsandbytes int8.
# All other settings as in stk_08.
...
class SUT:
    ...
    def load_model(self):
        ...
        import os
        os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"  # from stk_08
        self.model = LLM(
            self.model_path,
            dtype=self.dtype,
            tensor_parallel_size=self.tensor_parallel_size,
            quantization="bitsandbytes",     # ADDED stk_09: int8 weight emulation
            max_model_len=2668,             # from stk_03
            max_num_batched_tokens=2668,    # from stk_03
            gpu_memory_utilization=0.95,    # from stk_05
            kv_cache_dtype="auto",         # override for stk_09: let vLLM pick a supported KV dtype
            compilation_config=CompilationConfig(level=3),  # from stk_07
        )
