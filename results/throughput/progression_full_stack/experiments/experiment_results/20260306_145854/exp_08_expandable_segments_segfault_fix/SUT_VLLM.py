...
#   Was max_split_size_mb:512 — caps block reuse, causes fragmentation.
#   After ~30 batches allocator crashes. expandable_segments fixes this.
# CHANGE 2: Removed dead multi-GPU env vars (NCCL_* etc — no-ops on TP=1).
# CHANGE 3: dtype=float16 explicit (bfloat16 silently cast anyway for FP8).
# CHANGE 4: cpu_offload_gb=0 confirmed (was 10 in some variants — 75x PCIe penalty).
...
class SUT:
    ...
    def load_model(self):
        ...
        self.model = LLM(
            ...
            enforce_eager=False,
            cpu_offload_gb=0,              # CHANGE: confirmed 0 (was 10 in some variants — 75x PCIe penalty)
