# EXP 07: +max_num_batched_tokens=65536, gpu_memory_utilization 0.98 → 0.95
# CHANGE 1: max_num_batched_tokens 24576 → 65536 — removes prefill bottleneck
#   131072 causes illegal memory access in V1 engine profiler — hard ceiling is 65536
# CHANGE 2: gpu_memory_utilization 0.98 → 0.95 (REQUIRED alongside this change)
#   Larger batched_tokens spikes activation memory during profiling.
#   0.98 + 65536 together causes OOM at V1 engine startup. 0.95 gives safe headroom.
# Also adds: chunked prefill, prefix caching, enforce_eager=False, cpu_offload_gb=0
...
class SUT:
    ...
    def load_model(self):
        ...
        self.model = LLM(
            ...
            gpu_memory_utilization=0.95,  # CHANGE: reduced from 0.98 — required with larger batched tokens
            max_model_len=2668,
            max_num_batched_tokens=65536, # CHANGE: removes prefill scheduling bottleneck
            max_num_seqs=512,
