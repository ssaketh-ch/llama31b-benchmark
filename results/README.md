...
Single-flag sweep from the stock production baseline. Baseline: BF16, BS=16,
stock vLLM V1 defaults (exp_00 = 1082.82 tok/s).
...
| exp_01 | -prefix_caching=False | 1048.73 | -3.15% |
| exp_02 | -async_output_proc OFF | 1006.82 | -7.02% |
...
- Disabling V1 defaults (APC, async output) both regress throughput. Do not turn these off without a reason.
...
- fp8 W8A8 more than doubles throughput and halves latency end-to-end. Consistent with isolation.
- int8 W8A8 failed entirely (exit within 20s). Likely a missing kernel -- investigate before retrying.
- APC is strongly negative on CNN/DailyMail (+42% from disabling it) because the dataset has low prefix reuse. This is workload-dependent; APC is beneficial for chat/RAG with long shared system prompts.
- CP ON + MNBT=16384 is the optimal chunked prefill setting. CP OFF costs -14%; lower MNBT costs 2-4%.

---

### 1.3 Progressive Stacking

Each step permanently adds one flag on top of all previous ones.
...
| stk_09_bitsandbytes | +bitsandbytes (replaces fp8) | 446.14 | -59.03% | 1764.17 | 3787.78 | VALID |
| stk_02_bs1024 | +batch_size=1024 (standalone reference) | 3321.76 | +205.02% | 191.41 | 509.22 | INVALID* |

> *stk_02_bs1024: min_duration not satisfied. Increase expected QPS for a valid loadgen run.
> Throughput is directionally correct but not a certified result.
...
- expandable_segments (stk_08) drops to 1420 tok/s in the stacked context despite being positive in isolation -- negative interaction with fp8 memory allocation.
...
|---|---|---|---|---|
| acc_baseline | BF16 BS=16, all flags pinned | 38.8001 | 24.5197 | baseline |
| acc_ignore_eos_off | ignore_eos=False explicit | 38.8001 | 24.5197 | ~= baseline |
| acc_min_tokens_0 | min_tokens=0 | 38.8001 | 24.5197 | ~= baseline |
| acc_skip_special_tok | skip_special_tokens=True | 38.8000 | 24.5197 | ~= baseline |
| acc_temp_explicit | temperature=0.0 explicit | 38.8001 | 24.5197 | ~= baseline |
...
Shorter max_model_len truncates inputs and directly degrades ROUGE.

| Experiment | max_model_len | Tokens Kept | ROUGE-1 | ROUGE-L | Verdict |
|---|---|---|---|---|---|
| acc_kv_len_1000 | 1000 | 872 | N/A | N/A | FAILED -- too aggressive |
| acc_kv_len_1500 | 1500 | 1372 | 36.5577 | 23.2105 | **-2.24 R1 -- unsafe** |
...
Large batch sizes cause a ~13-point ROUGE-1 regression and ~16% shorter generation length.
...
|---|---|---|---|---|---|
| acc_max_tokens_64 | 64 | 41.6827 | 26.7254 | 3,990,098 | Higher ROUGE -- concise outputs |
| acc_baseline | 128 (default) | 38.8001 | 24.5197 | 8,164,523 | Reference |
| acc_max_tokens_256 | 256 | 27.5814 | 17.5390 | 16,624,600 | Lower ROUGE -- outputs exceed ref length |
...
APC as strongly negative on CNN/DailyMail (-42% cost to enable). The accuracy run confirms APC
is ROUGE-neutral on unique articles. **APC should be disabled for datasets with low prefix reuse
and enabled for chat/RAG workloads with long shared system prompts.**
...
|---|---|---|
| acc_fp8_quant failed -- no output | CRITICAL | Re-run; fp8 is the primary throughput opt and needs accuracy sign-off |
| combo_acc_full_optimal failed | CRITICAL | Re-run; this is the final accuracy answer for the production config |
| BS=1024 ROUGE regression (-13 R1) | CRITICAL | Investigate non-determinism source (attention ordering, padding, EOS behavior) |
...
| stk_02_bs1024 INVALID (loadgen) | MEDIUM | Rerun with corrected expected QPS for certified throughput result |
...
8. **Do not use bitsandbytes quantization** -- -59% in stacked context.
