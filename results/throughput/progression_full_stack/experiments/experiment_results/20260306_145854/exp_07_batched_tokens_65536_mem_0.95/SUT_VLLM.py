...
# CHANGE 1: max_num_batched_tokens=65536 — removes prefill scheduling bottleneck.
...
# CHANGE 3: enable_chunked_prefill=True — enabled alongside batched tokens.
# enable_prefix_caching remains pinned False — still not testing it.
...
class SUT:
    ...
    def load_model(self):
        ...
        self.model = LLM(
            ...
            gpu_memory_utilization=0.95,   # CHANGE: reduced 0.98->0.95 (required with batched tokens)
            max_model_len=2668,
            max_num_batched_tokens=65536,  # CHANGE: removes prefill bottleneck (131072 crashes V1)
            max_num_seqs=512,
            enable_prefix_caching=False,   # pinned off — still not testing this
            enable_chunked_prefill=True,   # CHANGE: enabled alongside batched tokens
            enforce_eager=False,
            cpu_offload_gb=0,
