# EXP 13: +kv_cache_dtype=fp8_e4m3  [vLLM 0.10.0]
# fp8_e5m2 is hard-blocked by vLLM when quantization=fp8 (attention/layer.py).
# fp8_e4m3 is the correct alternative — not blocked, higher precision.
# Memory effect identical: ~148 -> ~296 concurrent seqs.
# IMPORTANT: run accuracy check — ROUGE must still meet 99pct targets.
...
class SUT:
    ...
    def load_model(self):
        # CHANGE: kv_cache_dtype="fp8_e4m3" + compilation_config={"level": 0}
        #
        # Root cause (confirmed across 3 attempts):
        #   Any non-auto kv_cache_dtype causes vLLM to leave a data-dependent branch
        #   in the attention forward pass (attention/layer.py:239):
        #     if attn_metadata.enable_kv_scales_calculation:
        #   torch.compile (compilation_config=3, default in vLLM 0.10.0 V1 engine)
        #   cannot statically trace this branch regardless of calculate_kv_scales setting.
        #   torch._dynamo.exc.Unsupported: Data-dependent branching
        #   This is a vLLM 0.10.0 bug — kv_cache_dtype and torch.compile are
        #   fundamentally incompatible in this release.
        #
        # Workaround: disable torch.compile (level=0) for this experiment.
        # Cost: lose the kernel fusion benefit from exp_16. This isolates the KV
        # cache memory effect only. If exp_13 wins over baseline (exp_11),
        # the combined experiment (exp_14) can test whether compile+kv_cache works
        # in a future vLLM version or with enforce_eager=True.
        #
        # Memory: fp16 KV ~10.9MB/seq -> fp8 KV ~5.5MB/seq at max_model_len=2668.
        # ~148 concurrent seqs -> ~296 concurrent seqs.
        # IMPORTANT: run accuracy check — ROUGE must still meet 99pct targets.
        ...
        self.model = LLM(
            ...
            kv_cache_dtype="fp8_e4m3",          # CHANGE: fp8 KV cache
            compilation_config={"level": 0},    # CHANGE: disable torch.compile (incompatible with fp8 KV)
        )
